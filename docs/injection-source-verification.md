...
>
> **v1.5.0 注记（2026-09-08）**：机制命名更新为 **Induction（成员装配）**，§5 骨架化（8 块五层：A 主体定义/B 质量规范/C 状态调节/D 工作供给/E 任务指示，Metaphor 为渲染层不计入 8 块）。**纯术语改名——注入文本内容不变**，本核对文件目标（§5 全文与实现逐字一致）不受影响。
>
> **v1.6.1 注记（2026-09-15）**：§5 有两处按 dsp 实现改正，未来按 §5 全文重核时须按新口径：① **§5.7** ——入口 SKILL 为**原文直推、无包裹头**（无 `# CCC 入口技能：…（来源: …）` 抬头），多入口以**空行**分隔（原记"以 `---` 分隔"是把 `SKILL.md` 自身 frontmatter 的定界符误认作块间分隔符）；② **§5.12 新增**——轨迹声明的 skill 供给（条件注入：仅当轨迹在 `SESSION.md` frontmatter 声明 `skills:`），属 D 工作供给层的**条件供给、不计入 8 块骨架**；其**逐字规格不在本仓**（接口真相源 = dsp `docs/trajectory-skill-injection.md`），故本文件重核时该块只核对"存在与语义"，逐字项以该文为准。

> 目的：证明 §5 Induction（成员装配）内容与两个已投产实现逐字一致。本文件记录核对过程与源码位置，供未来实现（pi-serenity 等）核对。
...
v1.4.0 起**注入内容以 dsp v1.30.1 为权威实现**（specs README §5 全文 = 标准基线；v1.5.0 起机制称 Induction）。两实现 Induction 装配文本**语义一致**（CCE 完全一致；其余块仅动态字段差异）。§5 注入规范是公共语义提取，可作为任何新 ACC 实现的权威注入内容基线——**osp 侧按 v1.4.0/v1.5.0 同步后需更新本核对文件**。
