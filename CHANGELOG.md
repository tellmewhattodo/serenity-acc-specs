...
- **dsp 侧生产路径尚未跑过 CRO**（诚实标注）：机制已接线、`cro-guide` 已实调通，但**尚无任何轨迹写过 `continuous-re-occurrence.ts`** ⇒"判定 → 唤起"这一段在**生产**上零次执行（集成实测是在临时 CCC 内做的）。**这不影响本批的契约入标准**（specs 记的是契约），但**验收状态按事实记**
