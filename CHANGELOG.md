...
- **dsp 侧生产路径的验收状态（诚实标注；🔴 本行在 v1.8.0 提交后 **1 小时内即被现实更新**，按事实改写）**：
  **定稿时的写法** =「机制已接线、`cro-guide` 已实调通，但**尚无任何轨迹写过 `continuous-re-occurrence.ts`** ⇒ 判定这一段在生产上零次执行（集成实测是在临时 CCC 内做的）」。
  **提交后实测（同一会话，`00:3x`~`00:4x`）**：① **两条轨迹各自写出程序** —— S151（28.9 KB，`00:36:55`）与 S185（28.4 KB，`00:41:47`），**均为正式名**（非开发名）⇒ 按 §4.5.1「**文件在 = 启用**」，它们**即时生效**；
  ② 🔴 **ACC 的 5min tick 确已 spawn 它们**——判据 = 程序自写的状态文件 `cro-state.json` 里 **`lastRunAt` = `00:40:54.360`**，与同刻 `acc-diag` 读到的**调度器上次 tick 时间 `00:40:54.404` 逐毫秒对齐**，且内含**上上一次 tick（`00:30:54.398`）的判定记录** ⇒ **判定链在生产上至少执行两次**；
  ③ 其中一次判定 = **「已有在办唤醒条目 ⇒ 防双触发，不叫」**——即§4.5.3 那条把防抖推给程序的语义**在生产上正确生效**。
  **仍然不成立的**（精确边界）：`lastWakeRequestAt` 为 `null` ⇒ **`wake: true` 分支尚未在生产上触发过**；**所有者验收**亦未发生。
  ⇒ **结论**：**"ACC spawn 轨迹程序"这一段已真机验证**；**"判定为真则投递"这一段仍属未验证**。**这不影响本批的契约入标准**（specs 记的是契约）。
  **方法论（本行本身即实例）**：**标准里的"现状/验收"注记必须带日期戳**——它可以在标准提交后**几分钟**被现实推翻（本轮实证），而"零次执行"这类**否定式现状断言**是最易过期的一类。
